...
systemCallMethods.getBinaryFromSdkRoot = async function getBinaryFromSdkRoot(binaryName) {
    ...
    let buildToolsDirs = await (0, helpers_1.getBuildToolsDirs)(this.sdkRoot);
...
systemCallMethods.adbExec = async function adbExec(cmd, opts = {}) {
    ...
    opts.timeout = opts.timeout || this.adbExecTimeout || helpers_1.DEFAULT_ADB_EXEC_TIMEOUT;
    ...
    const execFunc = async () => {
        try {
            ...
            let { stdout, stderr } = await (0, teen_process_1.exec)(this.executable.path, args, opts);
...
systemCallMethods.createSubProcess = function createSubProcess(args = []) {
    ...
    logger_js_1.default.debug(`Creating ADB subprocess with args: ${JSON.stringify(args)}`);
    return new teen_process_1.SubProcess(this.getAdbPath(), args);
};
/**
...
systemCallMethods.launchAVD = async function launchAVD(avdName, opts = {}) {
    ...
    launchArgs.push(...((0, helpers_1.toAvdLocaleArgs)(language, country)));
...
systemCallMethods.waitForEmulatorReady = async function waitForEmulatorReady(timeoutMs = 20000) {
    if (await this.getApiLevel() >= 31) {
        ...
        try {
            ...
            await (0, asyncbox_1.waitForCondition)(async () => {
                try {
                    ...
                    if (lodash_1.default.includes(reason, NO_READINESS_SERVICE_ERROR)) {
                        hasReadinessService = false;
                        return true;
                    }
                    else if (!lodash_1.default.includes(reason, SUBSYSTEM_STATE_OK)) {
                        logger_js_1.default.debug(`Waiting for emulator startup. Intermediate error: ${err.message}`);
                    }
                }
                return lodash_1.default.includes(reason, SUBSYSTEM_STATE_OK);
            }, {
                waitMs: timeoutMs,
                intervalMs: 1000,
            });
        ...
        logger_js_1.default.info(`The device under test does not have reboot_readiness service. ` `Falling back to the alternative boot detection method.`);
    }
...
systemCallMethods.installMitmCertificate = async function installMitmCertificate(cert) {
    ...
    if (!lodash_1.default.isBuffer(cert)) {
        cert = Buffer.from(cert, 'base64');
    }
    const tmpRoot = await support_1.tempDir.openDir();
    try {
        const srcCert = path_1.default.resolve(tmpRoot, 'source.cer');
        await support_1.fs.writeFile(srcCert, cert);
        let { stdout } = await (0, teen_process_1.exec)(openSsl, ['x509', '-noout', '-hash', '-in', srcCert]);
        ...
        ({ stdout } = await (0, teen_process_1.exec)(openSsl, ['x509', '-in', srcCert], { isBuffer: true }));
        let dstCertContent = stdout;
        ({ stdout } = await (0, teen_process_1.exec)(openSsl, ['x509',
            '-in', srcCert,
            '-text',
            '-fingerprint',
            '-noout'], { isBuffer: true }));
...
systemCallMethods.isMitmCertificateInstalled = async function isMitmCertificateInstalled(cert) {
    ...
    try {
        ...
        await support_1.fs.writeFile(tmpCert, cert);
